...
```{python train, fig.cap = "Estimated relationships between x and abundance as training progresses. Dark blue lines represent predictions from early training iterations, and green/yellow represent middle/late training iterations."}
...
Recall that the detection probability $p$ was independent of our covariate $x$. 
How did the model do at representing this independence?

```{python p-plot, fig.cap="Estimated relationship between detection probability and the covariate (blue), and the actual relationship (black)."}
_ = plt.plot(x, p_hat.detach().numpy())
_ = plt.plot(x, np.repeat(pr_detection, nsite), color='k')
_ = plt.xlabel('Covariate value')
_ = plt.ylabel('Detection probability')
_ = plt.ylim(0, 1)
plt.show()
```
...
- Karpatne, Anuj, et al. "Physics-guided neural networks (pgnn): An application in lake temperature modeling." arXiv preprint arXiv:1710.11431 (2017).
- Raissi, Maziar. "Deep hidden physics models: Deep learning of nonlinear partial differential equations." The Journal of Machine Learning Research 19.1 (2018): 932-955.
