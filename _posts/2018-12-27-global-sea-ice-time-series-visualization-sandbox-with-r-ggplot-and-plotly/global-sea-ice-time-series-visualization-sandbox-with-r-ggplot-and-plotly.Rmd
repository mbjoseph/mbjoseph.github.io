...
description: |
  How to fetch sea ice data and visualize extent over time.
author:
...
get_seaice_data <- function() {
  ...
  northern_hem <- read_nsidc_csv("N_seaice_extent_daily_v3.0.csv") %>%
    mutate(hem = "Northern hemisphere")
  southern_hem <- read_nsidc_csv("S_seaice_extent_daily_v3.0.csv") %>%
...
p <- seaice %>%
  ...
  facet_wrap(~ hem, ncol = 1)
ggplotly(p)
```

This post is not sponsored by the National Snow and Ice Data Center, 
[which serves the data](http://nsidc.org/data/G02135). 
