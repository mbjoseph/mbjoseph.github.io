...
where the occupancy status in year 0, $z_0(i,j) \sim Bernoulli(\rho_{0i})$, and 
$\rho_{0i} \sim Uniform(0,1)$. $\beta_i$ and $\rho_i$ are parameters that 
...
hyperparameters such that $\beta_i \sim Norm(\mu_\beta, \sigma_\beta)$ and 
$rho_i \sim Norm(\mu_\rho, \sigma_\rho)$. We can generate occupancy data as 
follows:
...
$logit(p_i) \sim Norm(\mu_p, \sigma_p)$.
